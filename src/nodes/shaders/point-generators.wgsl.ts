 ...
 * "both, one implementation" convention — grid, line, circle, sphere, tube and torus
 * are the same dispatch with a different `shape` uniform, so switching shapes never
 * recompiles (§V5) and a fix lands in all six at once.
 *
...
export const POINT_GENERATOR_WGSL = `struct GeneratorParams {
  count: u32,
  shape: u32,          // 0=line 1=circle 2=grid 3=sphere 4=tube 5=torus
  cols: u32,
  rows: u32,
  sizeX: f32,
  sizeY: f32,
