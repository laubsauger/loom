...
describe("the fork the probe exists to split", () => {
  it("calls a surface that IS being blitted into, and reads black, painting black", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(handleWith({}), canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
    expect(reading.verdict).toBe("presenting-black");
    expect(reading.readback).toEqual({ luma: 0, alpha: 1 });
  });

  it("calls a surface no blit has reached recently NOT painting at all", () => {
    stubReadback([0, 0, 0, 255]);
    ...
    const reading = read(stale, canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
    expect(reading.verdict).toBe("not-presenting");
  });

  it("never reports the two as the same thing", () => {
    stubReadback([0, 0, 0, 255]);
    const box = { w: 640, h: 360 };
    const store = { w: 1280, h: 720 };
    const painting = read(handleWith({}), canvasWithBox(box, store)).verdict;
    const silent = read(
      handleWith({ lastPresentTime: NOW - 9_000 }),
      canvasWithBox(box, store),
    ).verdict;
    expect(painting).not.toBe(silent);
  });

  it("a surface that has NEVER presented is not painting at all, not merely black", () => {
    stubReadback([0, 0, 0, 255]);
    const never = handleWith({ lastPresentTime: null, presentedFrames: 0 });
    const reading = read(never, canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
    expect(reading.verdict).toBe("not-presenting");
    expect(reading.presentAgeMs).toBeNull();
  });

  it("says the GPU path is FINE when pixels are actually there, so the hunt moves downstream", () => {
...
describe("the three suspects T739 names, each with its own answer", () => {
  it("(a) §V659 — a remounted element whose surface was never configured", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(
  ...
  it("(b) §V658 — a 1x1 backing store beats every later verdict, healthy runtime or not", () => {
    stubReadback([0, 0, 0, 255]);
    ...
    const reading = read(handleWith({}), canvasWithBox({ w: 640, h: 360 }, { w: 1, h: 1 }));
    expect(reading.verdict).toBe("store-collapsed");
  });

  it("(b') a canvas with no laid-out box at all in its window", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(handleWith({}), canvasWithBox({ w: 0, h: 0 }, { w: 1280, h: 720 }));
    expect(reading.verdict).toBe("no-css-box");
  });

  it("a surface that outlived the device it was configured against (§V23)", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(
  ...
  it("an output that resolves to no source is not a popout fault", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(
  ...
  it("no handle at all reads as no handle, not as a black picture", () => {
    stubReadback([0, 0, 0, 255]);
    const reading = read(null, canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
    expect(reading.verdict).toBe("no-handle");
  });

  it("unreadable pixels are never folded into black", () => {
    stubReadback(null);
    const reading = read(handleWith({}), canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
...
describe("placement, and the one line the owner reads", () => {
  it("names the window the canvas is actually in", () => {
    stubReadback([0, 0, 0, 255]);
    const docked = read(handleWith({}), canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
    ...
    floated.width = 1280;
    floated.height = 720;
    expect(read(handleWith({}), floated).placement).toBe("floated");
  });

  it("carries the discriminating numbers, or it is not a report", () => {
    stubReadback([0, 0, 0, 255]);
    ...
    const line = formatViewerReading(
      read(handleWith({}), canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 })),
    );
  ...
  it("says so when the handle cannot describe itself, rather than inventing a runtime answer", () => {
    stubReadback([0, 0, 0, 255]);
    ...
    const reading = read(bare, canvasWithBox({ w: 640, h: 360 }, { w: 1280, h: 720 }));
