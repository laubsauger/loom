...
/**
 * T1271: the analysis is ONE instance and every lane is an expression on one of its two
 * bags — `lvl1` for the ranked levels, `hit1` for the drum counts. These two helpers keep
 * the reads legible at the call site; the ranges are what carries the meaning.
 */
...
export const novaTorusDocument = document(
  ...
  graph(
    [
      ...
      node("feedback1", "feedback", [1880, 220], { clearColor: [0, 0, 0, 1], reset: false, source: "null2", substeps: 1 }, { label: "feedback1", parameters: { persistence: expressionSlot(`0.62 + 0.16 * ${LEVELS("level")}`, 0.7) } }),
      node("hsv1", "hsv", [2320, -100], { saturation: 1.24, value: 1 }, { label: "hsv1", parameters: { hueoffset: drivenSlot("lfo1", 0) } }),
      node("level1", "level", [560, 200], { blacklevel: 0, contrast: 1, gamma1: 1, invert: 0, opacity: 1, whitelevel: 1 }, { label: "level1", parameters: { brightness: expressionSlot(`0.4 + 1.2 * ${LEVELS("low")}`, 1) } }),
      node("level2", "level", [560, 460], { blacklevel: 0, contrast: 1, gamma1: 1, invert: 0, opacity: 1, whitelevel: 1 }, { label: "level2", parameters: { brightness: expressionSlot(`0.15 + 4 * ${HITS("hatCount")}`, 0.15) } }),
      node("level3", "level", [1000, -280], { blacklevel: 0.01, contrast: 1, gamma1: 1, invert: 0, opacity: 1, whitelevel: 1 }, { label: "level3", parameters: { brightness: expressionSlot(`0.9 + 1.1 * ${LEVELS("low")}`, 1.45) } }),
      node("lookup1", "lookup", [1440, 112], { channel: "luminance", offset: 0, row: 0.5 }, { label: "lookup1", parameters: { scale: expressionSlot(`0.7 + 1.5 * ${LEVELS("highMid")}`, 1.45) } }),
       ...
       */
      node("pointgenerator1", "pointGenerator", [-782, 44], { cols: 256, count: 65536, radius: 1, rows: 256, shape: "torus", sizeX: 2, sizeY: 2, sizeZ: 2 }, { label: "pointgenerator1", parameters: { radius2: expressionSlot(`0.18 + 0.3 * ${LEVELS("lowMid")}`, 0.33) } }),
      ...
      node("renderpoints1", "renderPoints", [240, -100], { accumulate: false, blend: "additive", color: [0.17, 0.27, 0.54, 1], count: 65536, group: "" }, { label: "renderpoints1", parameters: { sizePixels: expressionSlot(`1.1 + 0.9 * ${LEVELS("level")}`, 1.55) } }),
       ...
       */
      node("renderpoints3", "renderPoints", [240, 460], { accumulate: false, blend: "additive", color: [0.22, 0.88, 1, 1], count: 65536, group: "p.velocity.x > 0.66 && p.velocity.y < 0.04" }, { label: "renderpoints3", parameters: { sizePixels: expressionSlot(`0.6 + 2.6 * ${HITS("hatCount")}`, 0.6) } }),
      ...
      /**
       * T1271 — ONE ANALYSIS INSTANCE, and the eight lanes became eight expressions.
       *
       * What stood here was `valuelag1` (0.09 s) into eight gain·bias pairs — sixteen
       * `valueMath` nodes conditioning ONE source's raw band energy. Measured over 30 s of
       * the shipped pattern, that arithmetic was making two separate claims it could not
       * support. The bands it multiplied are this pattern's, so every gain was fitted to
       * this pattern's spread: the `high` lanes ran NEGATIVE for 21-22% of the run (the
       * arrangement's quiet bar pulls the top end down, ×23.2749 − 7.5588 takes
       * `level2.brightness` to −6.30 at its worst, and a negative brightness SUBTRACTS the
       * cyan layer from the frame instead of dimming it). And six of the eight retained
       * values sat outside the range their own lane produced — §V914's shape — with three
       * of them at 0 on parameters that DRAW: cut the audio path and the shipped file
       * renders a BLACK FRAME, 0.0000 of it lit, because `renderpoints1.sizePixels`
       * retained 0 is a point cloud with no points.
       *
       * The envelope stays 0.09 — E35's own number, kept against E24's 0.08, because the
       ...
       * it is an event — `renderpoints3` selects the tips (`p.velocity.x > 0.66`) and
       * `level2` grades them, so the two together are one flash. A rank rests at 0.5
       * (§V952), which would leave the sparkle half-lit at all times; a count rests at 0
    ...
    [
      ...
      edge("e1-source1-analysis1", ["source1", "out"], ["analysis1", "audio"]),
      ...
      edge("e12-analysis1-lvl1", ["analysis1", "levels"], ["lvl1", "in"]),
      edge("e13-analysis1-hit1", ["analysis1", "hits"], ["hit1", "in"]),
