...
Use the exact experimental Electron binary selected and tested by the desktop track, currently recorded as 45.0.0-alpha.5 in [the local app README](../experiments/electron-app/README.md). Recheck the official release and matching headers before implementation; no automatic runtime downgrade. SharedArrayBuffer is CPU shared memory, not this experimental GPU API and not an automatic Python-memory mapping.
...
- `experiments/electron-app/README.md`: actual development shell still denies camera/microphone permissions and has no native graph integration. The synthetic texture probe is not an installed Syphon/Spout/NDI service.
