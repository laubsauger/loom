...
| **centre**, `open1` | `mode: "surface"` on the grid the **generator** published | a skin — and a slit. `pointsFromTexture` emits one point per lattice cell, so it already knows the adjacency and says `grid:96x96` on the edge; **a surface here needs no extra node at all**. The seam is open, so the cell between column 95 and column 0 is missing: **997 interior pixels in an eight-pixel-wide band**, and nothing else. Those 997 are the pixels where you see the *plate* — 125 of the tube's 557 rows; on the rest of the slit you are looking through the gap at the tube's own inner wall |
