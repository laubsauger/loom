...
| [E56 Vesper](./E56-Vesper.md) | THE INTENSITY OF THE MOMENT DECIDES THE TIME OF DAY (T1155): a sunset timelapse whose playhead is not a clock — an audio envelope drives `movieFileIn`'s Cue Point ABSOLUTELY and INVERTED, so loud is the sun still up and quiet is the sun gone, and a given phrase always lands on the same time of day. No new node: Cue plus a driven Cue Point is a scrub input, and an all-intra clip seeks at 58.9 completed seeks per second with zero superseded. Ships PORTRAIT at 720x1280 with an exposure/contrast desk (`tone1`/`grade1`) tuned for the footage. The first example that SHIPS ITS FOOTAGE — `public/media/portrait-sunset.mp4`, referenced relatively so it resolves under both bases — and the one that found T493's driven transport parameters had never worked |
