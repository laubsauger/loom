...
| [E54 Quorum](./E54-Quorum.md) | ONE OPERATOR, TWO JOBS (T1070): a force-directed layout IS gradient descent on the graph Laplacian, and diffusion over the same operator IS power iteration toward its low eigenvectors — so one loop lays four communities out AND finds the colour each one agrees on. The graph is the only authored thing (a stochastic block model keyed on identity); where the clusters sit, which bridges survive, how big each node draws and what colour it settles on all emerge. Built on `pointAt(slot)`, the neighbour read a point kernel did not have until this example needed it |
