...
| [E51 Chorus](./E51-Chorus.md) | ONE STREAM, NINE MOMENTS, AS A VJ INSTRUMENT: a video wall built from stock nodes — Tile makes the grid, SlitScan gives every cell its own delay out of a 61-frame history, a seeded per-cell tear breaks the loud cells (sparse by construction, held for a whole tick, and reproducible from the seed), and an evolving Ramp-into-Lookup palette ties the nine together. The TimeGrid library component: a texture in, a texture out, eight performance knobs, and the source is a PORT so the understudy, the webcam and a movie file feed it identically. The kick drives the BODY, not a knob — the cascade then turns one beat into a wave rolling across the grid, and a cell's own brightness arms its glitch. The grid is uniform-only, so a 3x3 becomes a 6x6 mid-show without reallocating the 69.75 MiB ring — asserted on the plan, because no picture can show it (T321, T242, §V228) |
