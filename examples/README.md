...
| [E51 Chorus](./E51-Chorus.md) | ONE STREAM, NINE MOMENTS: a video wall built from stock nodes — Tile makes the grid, SlitScan gives every cell its own delay out of a 61-frame history, and a duotone ties the nine together. The TimeGrid library component, whose whole interface is a texture in, a texture out and eight performance knobs; the source is a PORT, so the understudy, the webcam and a movie file feed it identically. The grid is uniform-only, so a 3x3 becomes a 6x6 mid-show without reallocating the 69.75 MiB ring — asserted on the plan, because no picture can show it (T321, T242, §V228) |
