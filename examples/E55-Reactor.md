...
An alien disco ball lit from inside: a churning core marched as an emissive volume inside an open organic skeleton of a few large cells per shell with finer shells within, struts that stand proud of the faces, an outer shell that breathes out of round while its lattice morphs, shutters that close on a drop so the ball collapses to contained light behind glass whose frames glow and breathe, then blazes back, a core that blows out on a swell, a camera that grazes the shells, dives through the centre and out the far side, and a palette that swings through its family every couple of minutes. Two `customWgsl` passes from one shader, every knob on the main node's page.
