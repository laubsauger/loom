...
An alien disco ball lit from inside: a churning core marched as an emissive volume inside an open organic skeleton of a few large cells per shell with finer shells within, struts that stand proud of the faces, an outer shell that breathes out of round while its lattice morphs, shutters that slam shut on a drop so the ball collapses to contained light behind inset translucent panels and blazes back, a core that blows out on a swell, a camera that grazes the shells, dives through the centre and out the far side, and a palette that swings through its family every couple of minutes. Two `customWgsl` passes from one shader, every knob on the main node's page.
...
A tour on the free-running clock, in six eased legs: wide; close on the shell surface; a graze along the outer shell, the eye just off it looking along the surface with the shell as the ground; one continuous dive through the centre and out the far side (never parked in the core); a graze along the middle shell from inside the outer one, the outer skeleton arching overhead; then a half-turn swing back at wide distance. The grazes are where Fresnel is strongest on every facet in view, which is why they were asked for and why they answer the "hard to see" note. The gaze holds its travel direction through the ball and turns back to the core once outside. `stations` scales how far from the wide shot the tour goes (0 holds the wide shot), `travel` is the seconds per tour, `orbit` the circling rate, and the exposure stops down as the eye nears the core. While the eye is inside the ball, plates read as glass and a plate on the lens is passed through, so the inside view is the skeleton around the core. Struts sweeping past the lens at close range is the strongest parallax there is.
...
One hue angle swings the core, the glass and the beams together: `hueSwing` degrees either side of the base colours at `hueDrift` degrees per minute, a swing rather than a full turn so the palette evolves through the family around orange and blue without passing through the yellow-greens a full circle lands on, and fast enough that it has visibly moved inside any real viewing. Each shell inward sits `shellHueStep` degrees further round from that same angle, so colour deepens toward the core and the shells keep their relationship at every moment. The sky does not turn.
