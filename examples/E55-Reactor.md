...
An alien disco ball lit from inside: a churning core marched as an emissive volume inside an open organic skeleton of a few large cells per shell with finer shells within, struts that stand proud of the faces, a lattice that morphs, shutters that slam shut on a drop so the ball collapses to contained light behind inset panels and blazes back, a core that blows out on a swell, a camera that dives through the centre and out the far side, and a palette that turns over minutes. Two `customWgsl` passes from one shader, every knob on the main node's page.
