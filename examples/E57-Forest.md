...
- The columns had to be **narrow enough to leave gaps** (T1170b; T1266 has since made them exactly the trunk — see below). At thirteen and four trunk radii every direction found an occluder and the picture went uniformly dark rather than striped. Seven and two and a half leaves lit lanes between the shadows, which is the whole effect.
...
### T1266: the shadow is the drawn tree's

The owner, on that file: the shadows "seem to be originating from a tree that is twice the size" of the one standing there. They were. A column seven radii wide with a black core of two and a half is a shadow cast by a far thicker tree: at nine metres, a 46 px black core behind a 22 px trunk. It also stood at the foot's radius all the way up, while the drawn stem tapered and leaned away from it.

Now the occluder is the stem as drawn. `moonVisible` reads the same four knots `buildTree` draws the trunk through, at the height the light ray passes, with lean, taper and swelling. One function, `stemKnot`, serves both, so the shadow cannot drift off its tree. The edge softens the way a disc light's does: half dark exactly on the silhouette, with the penumbra opening at tan(moonSize) per metre behind. The claims test checks this against the cause. One metre behind a foot, the shadow's half-dark width and centre equal the drawn trunk's cross-section, to a texel.

The price is the darkness the wide columns bought, and the owner chose to pay it. A wood of true-width trunks takes **5.2%** of the moonlight out of the fog (0.9476 wood against bare), where the wide columns took 22%. The frame came out a third brighter, so the shadowed term's coefficient went from 1.95 back to **1.0**. That returns the mean over frames 0, 300, 900 and 1500 to 68.59 of 255, against 68.17 before. The swing is narrower (60 to 79 against 55 to 100 on those frames), because thin shadows put less light coming and going into a walk. A half-way version (black on the trunk, a faint wide falloff) was rendered and rejected: every variant dark enough to pass the old bound brought the wide column back. It costs **+0.27 ms a frame** at 1280×720 (3.90 against 3.63 in the same quiet alternating run, so the ratio to E13 went 1.46 to 1.57). Most of that is the thin shadow itself: the shadow walk stops once the light is gone, and a wide black core used to put it out early.

Two claims had been crediting the wide columns and were re-derived. "The trees are the structure" now reads 0.725 (bare against wooded detail), where it was 0.545: the bare band did not move, and the wood lost the column edges it never owned. The snag balance is now measured with the volumetric off, where it reads 2.250 before and after, because a snag is geometry and T1266 did not touch the geometry.
