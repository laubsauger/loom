...
T393|.|fix B51 — ownership guard ∀ the floated pane window: module-level `Map<name, token>`, cleanup closes ONLY while it holds the token. keep BOTH properties that collided (named reuse ∀ focus-⊥-stack; deferred close ∀ the dock's adopt-back). gate ! simulate the DOUBLE MOUNT (mount→cleanup→mount w/ a name-reusing fake `open`) & assert the window is STILL OPEN — a single-mount test passes today & proves nothing|V334,V220,B51
T394|.|**FULLSCREEN the viewer.** Fullscreen API on the output/viewer surface, w/ Escape returning & the control ∈ the keymap (V78/V307) ⊥ hardcoded. ! survive the float-to-window path (T393) — a floated viewer fullscreening its OWN window is the useful case|V78,V307,T393
