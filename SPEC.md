...
T432|.|**capture STATUS surface** — mic DENIED | url FAILED park the capture ∈ an error status the hook exposes & ⊥ renders it. the moment a user dismisses their 1st mic prompt, every audio-driven parameter silently reads 0 w/ ⊥ explanation anywhere. V288 gap|V288,T414
...
T434|.|**audio DEVICE SELECTION + audio FILE input** ← owner, ∀ the T414 track. an `audioIn` today configures source mic|file & url; needs (a) an ENUMERATED device picker (`enumerateDevices`, labels only appear AFTER permission — say so ⊥ show blanks) & (b) a file input as the analog of `movieFileIn`. pairs w/ T432's capture-status surface: a device that vanishes mid-session ! say so|V288,T414,T432
T435|.|**DECIDE: do we need TD's SAMPLES?** owner asks whether CHOP-style per-sample channels are needed now that audio & CPU-number work exist. RECOMMENDATION (mine, to confirm): **⊥ adopt them.** our value graph is 1 scalar per channel per FRAME (control rate) & audio-rate DSP already lives INSIDE the analyser, which is the correct split — audio rate ∈ the audio engine, control rate ∈ the graph. what we ACTUALLY lose is narrow & fixable w/o a sample model: (a) SUB-FRAME onset timing — an onset between frames is late by ≤16ms ∴ fix by reporting per-frame onset COUNT & MAX, ⊥ only the instantaneous value; (b) HISTORY/trail ops — a bounded ring of last N values, ⊥ a general sample rate. adopting samples means every value node gains a rate, a window & resampling semantics ∴ a large permanent cost ∀ a narrow gain|V143,T414
