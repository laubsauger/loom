...
T437|.|**`onsetCount` + `onsetMax` ∈ AudioFeatures — & it ! land BEFORE T431.** the analyser is read ONCE per displayed frame ∴ intra-frame onsets are lost; recovering them needs an AudioWorklet accumulating BETWEEN reads (app-side, never touches the seam). TIMING IS THE POINT: new AudioFeatures fields are a V352 VERSIONING EVENT ∀ every recorded track — T431 has ⊥ shipped ∴ there are ⊥ tracks to invalidate YET. cheap now, costs a track version after|V352,T431,T435
