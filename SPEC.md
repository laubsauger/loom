...
V220: **"built, tested, ⊥ wired" is this project's DOMINANT failure mode — 12 instances.** B12 agent surface, T264 media, B23 render_preview, B25 analyze channels, B26 recompile classifier, `ContextMenuHost` (every menu the menus track built was unreachable while its suite stayed green), `createValueGraphSession` (B27), `createShaderCompilePipeline` (B28), component FLATTENING (B29), `grants.grant` (fixed inline), `PointerSource.set` (B30), `setCookPolicy` (B32). ∀ had green unit tests. the shared cause: a test SUPPLIES the wiring it is testing ∴ the 1 thing it cannot observe is whether anything else supplies it. T306's enumeration is the structural answer & ! outlive the tasks that prompted it.
...
T324|.|**publish the pointer** (B30) — `PointerSource.set` has ⊥ caller ∴ `FrameEvaluationInput.pointer` is always `{0,0,0}` & EVERY shader reading the shared block's pointer gets zero. normalise per V236, 1 publisher|V236,V182,V220
