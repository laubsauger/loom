...
T414|.|**AUDIO INPUT is missing entirely** — ⊥ audio node ∈ the catalogue, ∴ the single most characteristic realtime-visuals demo (audio-reactive) is unbuildable. `analyze` already proves the texture→scalar→parameter path; audio would feed the SAME value graph. spec: WebAudio source + band/level channels → `value` ports|V144,T407
