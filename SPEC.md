...
T320|.|**pass `components` to `compileGraph`** ∈ the app (B29) + the V210(c) trigger: a component-catalogue edit ! recompile host documents that instantiate it|V82,V210,V220
...
T314|.|agent `set_parameters` takes a bare value ∴ mode-setting works through `apply_graph_patch` & ⊥ through the tool named for it (V107, V215)|V215,V107
T315|.|`setViewport` via an agent MOVES THE HUMAN'S CAMERA — make it a V38 grant class, ⊥ ambient. 1 line to deny today|V38,V215
