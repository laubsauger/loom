...
T446|.|**OPEN QUESTION ∀ the owner — camera/light as PORTS vs TD's REFERENCES.** owner noted TD does it w/ REFERENCES on the Render node (paths typed into a parameter), which the T376 design already identified as a DELIBERATE divergence. we chose PORTS ∀: T350's dashed refs mean CYCLE-BREAKING ⊥ general indirection; names are rename-churned (the B41/B44 class); ports get V13 typing free; the owner asked to SEE the pipeline. TD's model's REAL advantage, ⊥ taste: it SCALES — a 20-object scene is 20 CONVERGING WIRES under ports & a path pattern under references. ∴ revisit IF scenes get large; CHEAP to change now, expensive after T377 ships|T376,T377,V131T444|.|**E25 Stage — the MULTI-STAGE render the owner asked for.** scene A (geometry + camera A + lights) → RENDER → a texture; that texture becomes a MATERIAL MAP on geometry ∈ scene B; camera B renders scene B → screen. a virtual SCREEN INSIDE a scene, the TD/Notch classic. everything driven — camera B orbits, scene A animates, a light moves. DEPENDS ENTIRELY on T377 + T428: today a render result IS a texture & CAN feed 2D ops, but there is ⊥ way to put it ON geometry, & ⊥ light to move. ∴ blocked, ⊥ deferred|T376,T377,T428,T429
