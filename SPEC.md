...
T443|.|`ui.findInGraph` (`mod+f`) — a DESIGN task, ⊥ a small command: ⊥ find surface exists. `mod+k` searches COMMANDS; `tab` adds an OPERATOR; this is searching EXISTING NODES & jumping to one. per V354 the binding ! be UNBOUND until the surface ships — the graph is visibly right there, ∴ a dead `mod+f` reads as broken. `e` openShaderEditor is EXEMPT: T436's dock-reveal seam lands it imminently|V354,T436,T440
...
T441|.|fix B68 & the gate that missed it: give `ui.toggleReferenceLines` an invoker (V153 promises a control), & extend the seam gate to ask what INVOKES each COMMAND — a keybinding, a menu row | a rendered control — ⊥ merely whether its registrar module is reached. B68 is the shape V318's rule cannot catch|V356,V318,B68
