...
T377|.|build T376: `camera` (eye/lookAt/fov/near/far/ortho — the V198 matrix composed where CONSUMED, since render knows aspect & camera does ⊥ know the target) · `light` (directional|point, colour, intensity; render's `lights` VARIADIC, V131-ordered, fixed MAX_LIGHTS=4 array + live count) · `geometry` = the ASSIGNMENT stage {points, material} → `scene` · `render` {scene VARIADIC, camera, lights VARIADIC, environment?} → texture2d.
