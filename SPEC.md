...
T373|.|**geometry/points nodes have ⊥ PREVIEW.** `torus1` renders ⊥ tile @ all — a generator whose whole job is SHAPE shows nothing, while the renderer downstream shows the shape. preview a pointset as its own thing (point splat | bounds+count), ⊥ borrow the renderer's image|V85,T352
