...
T398|.|**RELAY TRANSPORT + TOKEN — the missing half.** `webmcp.ts` implements the PROVIDER side (`navigator.modelContext`), which is a PROPOSAL: feature-detected ∴ registers NOWHERE ∈ a browser lacking it. webmcp.dev's flow is different — page connects OUT to a relay w/ a TOKEN, an external MCP client (Claude Desktop) attaches to the same relay, & the LIVE TAB's tools become that client's tools. ∴ ! add: paste-token UI, outbound connect, reconnect, disconnect, & the SAME agent surface over that transport (V39: 1 surface, many transports)|V39,V192,T397
