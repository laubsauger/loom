...
T357|.|**E9 & E10 FAIL the cook oracle** — `Pass "out#out:present" references unknown resource "points:sim:out"`. 2 COMMITTED examples failing a release gate (V89). points plumbing, ⊥ settings|V89,V220
T355|.|**phase-carrying frame pacer** (V298) — the driver stops passing `fps` to `backend.loop` & gates itself w/ an accumulator + half-tick tolerance. 120 Hz locks to every-2nd-tick = exact 60; 60 Hz every tick is due. unit-testable w/ a fake loop @ 8.33 ms. + vgpu gaps entry: the built-in cap quantizes on any display faster than the target|V298,V299,V176
