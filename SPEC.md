...
T345|.|**Output node preview** (V274) — the final picture, currently an empty body|V274
T346|.|**webcam shows nothing & pushes nothing downstream** — owner-reported. leading hypothesis: `copyExternalImageToTexture` extent mismatch ∈ the window between `play()` resolving & the `resize` event that sets the node's resolution override; if the upload fails silently there it stays black forever (V240 — a failure that reaches ⊥ diagnostic)|V240,V167,T312
