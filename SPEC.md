...
T367|.|**pointer ∈ `PointCtx`.** kernel sees `index,count,time,delta,frameIndex` ⊥ pointer, ∴ "swarm follows cursor" — the most obviously playable thing a point graph wants — is INEXPRESSIBLE. shared frame block already carries it; value graph already carries it. add to PointCtx & prove w/ an example|V182,T363
...
T369|.|per-point colour is `renderPoints` ONLY ∴ E13 had to choose spectral 2D sprites over LIT 3D solids — a prism needs colour to BEND & dispersing a uniform render gives grey fringes. extend the mapped-colour path to `renderInstances`|T364,T363
