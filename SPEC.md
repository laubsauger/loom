...
T392|.|fix B48 — transport commands ! register w/o a backend (they are TIME, ⊥ GPU), | the top bar's play/step ! refuse visibly instead of silently doing nothing. today a GPU-less machine has 2 dead keys & 2 dead buttons|V307,V288,B48
...
T439|.|**`inspector` & `viewer` declare a keymap context & CANNOT HOLD FOCUS** — the identical B66 defect, LATENT. ⊥ dead today ONLY because ⊥ binding names them, & the new mechanical gate goes RED the moment 1 does. fixes B67 symmetrically: whichever pane is clicked takes focus ∴ graph bindings stop firing elsewhere. CAREFUL — focusing on pointer-down ∈ the inspector changes blur/commit ∀ every parameter control|V351,B66,B67
