...
V256: **a baseline DEFAULT is ⊥ a ceiling.** `maxStorageBuffersPerShaderStage = 8` is what a device gets when you ask for NOTHING; desktop adapters offer 10-64+. we pass `requiredFeatures` & never `requiredLimits` ∴ every kernel is capped @ the floor on hardware that would give 8× more. SAFE to raise because `describeCapabilities` reads `gpu.device.limits` — the NEGOTIATED value, ⊥ the adapter's — so the report & the validator (T328) follow automatically. request `min(adapter.limits[k], what we need)`, NEVER blindly: over-requesting FAILS device creation outright.
...
T339|.|**two-pass spawn hook** — copy first (fits the budget), then an in-place `spawn(child, ctx)` kernel over just the NEWBORN range: the child ARRIVES as its parent's copy so inheritance is the initial value, n+2 bindings. design w/ T287|V257,T287,T323
