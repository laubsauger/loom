...
T337|.|**decide which shader pane the app mounts** (B35) — `ShaderEditorPanel` is rendered by nothing; the app fills the slot w/ `ShaderPane` wrapping `ShaderEditor`, while `shader-editor/index.ts`'s own docblock claims otherwise. same shape as B34 ∴ a product decision|V242,V253
