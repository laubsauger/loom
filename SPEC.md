...
T404|.|**SPLIT IDENTITY FROM ROLE — the refactor everything else needs.** `PaneId` is a fixed union of 10 NAMED SINGLETONS (`layout-storage.ts:25`) ∴ identity ≡ role & you can ⊥ have 2 viewers, ⊥ split arbitrarily. replace w/: layout = a TREE of SPLIT nodes {direction, ratio} & LEAF nodes {paneKey (minted instance id), role (1 of the 10)}. many leaves ? share a role. blast radius is small & total: `layout-storage`, `app-shell`, `dock-zone`, `pane-portal`, `pane-window`, `app.tsx` — 6 files, ∀ keyed on PaneId today|V15,V311,T405,T406
