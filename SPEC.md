...
T399|.|package.json script ∀ `src/mcp/serve.ts` — the headless stdio MCP server EXISTS & is the path that works w/ Claude Desktop TODAY, but there is ⊥ documented invocation ∴ ⊥ one would find it. add the script & the client-config snippet ∈ the help panel|T290,T397
