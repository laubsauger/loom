...
T362|.|**E12 Fluid** — a small advect/diffuse loop. E2 is reaction-diffusion (a chemistry); a FLUID is a velocity field carrying a dye, & the visible difference is that it FLOWS rather than blooms. mouse as the stirring force (V182's pointer, ⊥ a 2nd listener) so it is playable, ⊥ merely watchable|V88,V89,V182
T363|.|**E13 showcase** — the ambitious one. dispersion|prism-style refraction, driven by MOUSE + LFO + the value graph, using instancing where it earns its place. the twist ! be OUR capability: reference vgpu.sh's sample for the LOOK, ⊥ the implementation. this is the piece that shows what the tool IS ∈ one screen|V88,V89,V182,V273
