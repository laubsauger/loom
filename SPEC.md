...
T318|.|`trigger` & `lag` violate V194 (unprefixed beside `valueSlope`/`valueFilter`/`valueLimit`/`valueMath`)|V194
...
B27|2026-08-30|**the VALUE GRAPH session has ⊥ caller.** `createValueGraphSession` appears only ∈ its own tests though T273 is marked done ∴ `mouse1 → lag1 → parameter` may ⊥ actually run ∈ the product, & the value nodes declaring `stateful.reset` have nothing able to reset them. found while auditing reset coverage, ⊥ by a test. 7th instance (V220)|V220,V193
