...
V220: **"built, tested, ⊥ wired" is this project's DOMINANT failure mode — 9 instances.** B12 agent surface, T264 media, B23 render_preview, B25 analyze channels, B26 recompile classifier, `ContextMenuHost` (every menu the menus track built was unreachable while its suite stayed green), `createValueGraphSession` (B27), `createShaderCompilePipeline` (B28), component FLATTENING (B29 — the starter set ships & cannot compile). ∀ had green unit tests. the shared cause: a test SUPPLIES the wiring it is testing ∴ the 1 thing it cannot observe is whether anything else supplies it. T306's enumeration is the structural answer & ! outlive the tasks that prompted it.
...
T319|.|`useFrameLoop` takes an OPTIONS OBJECT (V221) — 7 positional params w/ 2 callbacks is 1 insertion away from silently swapping riders|V221,V222
...
T237|.|**Cache — `ring` resource kind + rotate pass, FIXED TAPS only.** COSTED: ⊥ a 3D texture — a ring is `pingPong` generalised 2→N (V226). fixed taps re-point an ordinary `texture_2d` binding per frame ∴ **⊥ WGSL change, ⊥ new binding type, ⊥ capability question**. beats chained Feedback strictly: chaining costs 2 textures PER TAP, a ring costs 1× (t-3 = 95 MiB & 3 nodes chained, vs 63 MiB & 1 node @ 4 slices). defaults `frames:8` `scale:1/2` ≈ 32 MiB (V228). resize invalidates the WHOLE ring (V62b), a stated consequence @ 60 slices|V226,V227,V228,V229,V62b
