...
T376|.|**SPEC the 3D pipeline: Camera, Light, Render.** today `renderInstances` carries eye/lookAt/FOV/near/far as its OWN parameters ∴ camera is ⊥ shareable, ⊥ animatable as a unit, ⊥ reusable across renderers, & there is ⊥ lighting model @ all (flat colour). TD/Notch analog: geometry is a THING, camera is a THING, light is a THING, RENDER consumes {geo, camera, lights} → texture. + AMBIENT & ENVIRONMENT (env map ∀ PBR, T428). decide port shapes & whether camera/light are `value`-like refs | real ports|V107,T377
T377|.|build T376's pipeline: `camera` node (eye/lookAt/fov/near/far/ortho), `light` node (directional+point, colour, intensity), `render` node taking geometry + camera + lights → texture. MIGRATE `renderInstances`/`renderPoints`: their inline camera params keep working (V311 — migrate BOTH directions) | convert to a generated camera node|V311,T376
