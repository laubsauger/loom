...
T401|.|**POINTSET PROCESSOR — `pointKernel` gains an optional pointset INPUT** (B57). minimal shape, measured by the T367 track: codegen needs ⊥ change; only `points.ts` buffer resolution — ∀ `role:"in"` binding whose attribute the incoming payload carries w/ matching type, bind the UPSTREAM half instead of the node's own read half; `out_*` keeps writing the node's own pairs. ~30 lines + refusals ∀ capacity/type mismatch. spec consequences: V197 ownership, capacity coherence, the `requires` contract, a manifest port change. unblocks T378 AND a T369 example|V197,B57,T378
