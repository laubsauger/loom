...
T368|.|**monotone expressions CLAMP+WARN silently @ t>0.** `transform.r` ±360, `ramp.phase` ±1 ∴ `time*k` is correct @ t=0 & broken @ t=52, & the warning FAILS the example gate. `%` is the fix & ⊥ points you at it. ∴ either WRAP bounded-angular/phase params instead of clamping, | diagnose w/ the `%` remedy named|V240,T363
...
T370|.|expression grammar v1 = ARITHMETIC only (⊥ `sin`, ⊥ `clamp`). ∴ expressions buy LOCALITY, ⊥ reach — an LFO can produce anything v1 can. decide: widen w/ a small function set, | state the locality-only purpose ∈ the help panel so ⊥ one expects reach|T363
