...
T325|.|**output ports on LFO/Constant/Timer** (B31) — the trio declare ⊥ ports ∴ ⊥ value chain can be wired from a moving source|V237,V180
...
B30|2026-08-30|**`PointerSource.set` has ⊥ caller.** created ∈ `useFrameLoop` & never fed; its own docstring says "React components own the events and call `set()`" & ⊥ component does ∴ `FrameEvaluationInput.pointer` is permanently `{0,0,0}` — every shader reading the shared block's `pointer` gets zero, & Mouse reads that same zero|V220,V182
B31|2026-08-30|**the value TRIO have ⊥ output ports.** LFO/Constant/Timer declare none ∴ Mouse is the only WIRABLE value source. w/ B30 (Mouse reads a frozen zero): **⊥ value chain can produce a CHANGING number ∈ the running app today.** edges traverse, stages evaluate, state integrates, reset works — & the only available source is a constant|V237,V180
