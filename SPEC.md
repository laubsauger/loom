...
T436|.|**NAMED LAYOUTS: save · name · update · restore · delete.** owner's ask. today `layout-storage.ts` persists ONE layout; this makes it a NAMED SET w/ a current selection. MIGRATION both directions (V311): the existing single stored layout loads as a named default & ⊥ user loses their arrangement. UPDATE ≠ SAVE-AS — an "update" that silently creates a 2nd entry is how a layout list becomes 40 near-duplicates. ships built-in presets ⊥ only user ones, & T426's default is the 1st preset|V311,T426,T404
...
T426|.|**DEFAULT LAYOUT: right sidebar FULL HEIGHT**, ⊥ truncated by the bottom bar, split HORIZONTALLY — viewer TOP, inspector/params BOTTOM. owner's ask & it is the better use of the space. lands ∈ the existing shell; the ARBITRARY split tree is T404 (parked). do ⊥ block this on that|T404,T379
