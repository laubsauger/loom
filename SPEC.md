...
V210: the compile gate has ≥2 independent structural triggers, & the document revision is only ONE. opening|closing a preview changes the SINK SET, ⊥ the document ∴ `classifyEdit` cannot see it & a gate keyed solely to revisions stops materializing a sink the moment someone opens a preview. an input that produces ⊥ document edit goes invisible to a document-keyed gate.
...
T311|.|retry dirty PREVIEW builds @ FRAME ENTRY, ⊥ @ compile (V209). lands BEFORE T308 ∴ the editor track ⊥ inherit a backend regression it cannot fix|V209,V258
