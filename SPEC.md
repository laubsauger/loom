...
T415|.|fix B60 — INLINE rename ∈ the node header: double-click | `n` opens an editor ON the title, Enter commits, Escape cancels, refusal on collision (V325 — an explicit name is REFUSED ⊥ suffixed). ⊥ a modal: the owner asked to "edit name directly in the header bar"|V325,V342,B60
T416|.|show the node TYPE beside its name, w/ a SETTING to hide it — owner: renaming loses the at-a-glance identification the type gave. TD shows the operator type. default ON; ! ⊥ clutter (V90) ∴ secondary weight, ⊥ a badge competing w/ the name|V90,T415
...
T390|.|project-settings inputs ⊥ match the app's controls — wrong bg/greys, ⊥ resembling `ui/controls`. a settings dialog that looks like a DIFFERENT app is exactly the drift T356 deleted a duplicate surface to prevent. ! reuse the shared control primitives, ⊥ restyle to match|V90,T356
