...
T359|.|project settings opens via a COMMAND, ⊥ a `useState` toggle (V307) — today it is unreachable from the palette & from any keybinding|V307,V78
T360|.|**keybinding EDITOR** — the keymap is live & nothing lets a user change a binding. the help panel's shortcuts tab already lists them FROM the resolved keymap; editing belongs where the list already is|V52,V307
T361|.|re-point T356's dialog|settings half @ the COMMAND REGISTRY once T359 lands, replacing the naming convention (V307). the pane half already enumerates against `AppShellProps` & keeps that|V307,V241
